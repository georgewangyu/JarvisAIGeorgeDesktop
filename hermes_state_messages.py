...
class SessionMessagesMixin:
    ...
    def mark_latest_turn_failure(self, session_id: str, user_row_id: int, surface: Dict[str, Any]) -> bool:
        ...
        user input identified by its durable row id, never a reply from an earlier turn. Keep other display
        metadata (notably reactions) intact.
        """
        if not session_id or not isinstance(user_row_id, int) or user_row_id <= 0 or not isinstance(surface, dict):
            return False
        ...
        def _do(conn):
            row = conn.execute(
                ...
                "AND id > ? AND ? = (SELECT MAX(id) FROM messages "
                "WHERE session_id = ? AND role = 'user' AND active = 1) "
                "ORDER BY id DESC LIMIT 1",
                (session_id, user_row_id, user_row_id, session_id)).fetchone()
            if row is None:
                return False
